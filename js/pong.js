...
this.CHARS = [
    ...
    context.fillStyle = 'rgba(118, 118, 118, 0.76)';
...
this.reset();
    ...
    drawScore()
    {
        const align = this._canvas.width / 3.28;
        const CHAR_W = this.CHAR_PIXEL * 2;
        this.players.forEach((player, index) => {
            const chars = player.score.toString().split('');
            const offset = align * 
                           (index + 1) - 
                           (CHAR_W * chars.length / 8) +
                           this.CHAR_PIXEL / 2;
    ...
    update(dt) {
        ...
        if (this.players.top < 0 || this.players.bottom > this._canvas.height) {
